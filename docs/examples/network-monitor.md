---
title: network-monitor — primary + secondary observer pattern
description: Walkthrough of the network-monitor example — a two-tier passive monitor pattern where a primary controller pushes the pipeline and injects synthetic traffic and a separate secondary controller demonstrates loadPipeline. Includes the BMv2 PacketIn primary-only delivery caveat.
keywords: [jp4, example, network-monitor, secondary, Flow.Publisher, loadPipeline, mastership, observability]
---
...
**Source on GitHub**: [`examples/network-monitor/`](https://github.com/zhh2001/jp4/tree/main/examples/network-monitor/) (Java + P4 + Gradle build)
...
Optional: `--args="my-bmv2-host:50051"` to override the device address. See the [example's README](https://github.com/zhh2001/jp4/blob/main/examples/network-monitor/README.md) for the BMv2 setup line.
...
- Comment out the `monitor.loadPipeline()` call and re-run. PacketIns will arrive but be dropped during dispatch (see the SLF4J warn: "no pipeline bound; dropping PacketIn") — see [Troubleshooting: no pipeline bound](/troubleshooting/no-pipeline-bound).
...
- [PacketIn observation from a secondary controller](/cookbook/packet-in-secondary) — the recipe extracted from this example.
- [Connection and arbitration](/guides/connection-and-arbitration) — primary vs secondary, election ids, the §6.4 read-without-primary contract.
- [Packet I/O](/guides/packet-io) — `packetInStream()` mechanics and the BMv2 §16.1 caveat.
- [Troubleshooting: PacketIn handler never fires](/troubleshooting/packet-in-not-firing) — the §16.1 gap as a symptom-first guide.
