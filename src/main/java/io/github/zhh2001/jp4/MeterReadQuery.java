...
/**
 * Builder for one P4Runtime {@code Read} request against a meter array,
 * returned by {@code P4Switch.readMeter(String)}. Optionally narrow the
 * read with {@link #index(long)} (server-side filter via the wire
 * {@code Index} field) and/or {@link #where(Predicate)} (client-side
 * filter applied after fetch), then call a terminal: {@link #all()},
 * {@link #one()}, or {@link #stream()}.
 *
 * <p>An empty filter set means "every cell in the meter array". Index
 ...
 * {@link CounterReadQuery#where(Predicate)} uses for counter reads.
 *
 ...
 * {@code MeterReadQueryImpl} returned by {@code P4Switch.readMeter} is
 * the canonical implementation.
 *
 * <p>Threading model mirrors {@link CounterReadQuery} exactly: terminal
 * operations are dispatched on the switch's outbound executor; results
 * return to the calling thread. A {@code MeterReadQuery} instance is a
 * mutable builder; confine to a single thread.
 *
 * @since 1.4.0
 */
public interface MeterReadQuery {
     ...
     */
    MeterReadQuery index(long index);
     ...
     */
    MeterReadQuery where(Predicate<? super MeterEntry> filter);
     ...
     */
    Optional<MeterEntry> one();

    /**
     * Streams matching cells; the underlying gRPC iterator closes when the
     * stream closes. Always use with try-with-resources.
     */
    Stream<MeterEntry> stream();

    CompletableFuture<List<MeterEntry>> allAsync();
    CompletableFuture<Optional<MeterEntry>> oneAsync();
}
